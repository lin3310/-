...
@Component({
  ...
  template: `
    <div class="flex flex-col h-full bg-[var(--vibe-bg-main)]">
      ...
      <div class="p-6 text-center border-b border-[var(--vibe-border)]">
        <h2 class="text-2xl font-bold text-[var(--text-primary)]">{{ wf.t('crys.title') }}</h2>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        @if (isLoading()) {
          ...
          <div class="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
            @for (card of personaCards; track card.key) {
              <div class="bg-[var(--vibe-bg-card)]/80 backdrop-blur-sm rounded-2xl shadow-lg border p-5 flex flex-col animate-fadeInUp border-[var(--vibe-border)]" 
                ...
                @if (editingKey() === card.key) {
                  <textarea [(ngModel)]="editableContent" class="flex-1 w-full bg-[var(--vibe-bg-input)] border rounded-lg p-2 text-sm focus:ring-2 outline-none border-[var(--vibe-border)] focus:ring-[var(--vibe-accent)] text-[var(--text-primary)]"
                            rows="8"></textarea>
                } @else {
                ...
                <div class="mt-4 flex justify-end gap-2 items-center">
                   @if (cardLoading() === card.key) {
                     <div class="text-xs animate-pulse font-medium text-[var(--vibe-accent)]">{{ wf.t('common.loading') }}</div>
                      ...
                      <button (click)="regenerate(card.key)" title="Regenerate" class="p-2 rounded-full transition-colors opacity-70 hover:opacity-100 bg-[var(--vibe-bg-header)]">
                        <app-icon name="autorenew" [size]="20" class="text-[var(--vibe-accent)]"></app-icon>
                      </button>
                      <button (click)="startEdit(card.key)" title="Edit" class="p-2 rounded-full transition-colors opacity-70 hover:opacity-100 bg-[var(--vibe-bg-header)]">
                        <app-icon name="edit" [size]="20" class="text-[var(--vibe-accent)]"></app-icon>
                      </button>
                   }
                </div>
              </div>
       ...
       <div class="p-6 bg-[var(--vibe-bg-card)]/50 border-t flex justify-between items-center border-[var(--vibe-border)]">
        <!-- Modify (Back) -->
        <button (click)="goToModify()" [disabled]="!structuredPersona()" 
            ...
            {{ wf.t('crys.btn_modify') }}
        </button>
        
        <div class="flex justify-center gap-4">
            <!-- Refine (Director) -->
            <button (click)="goToRefine()" [disabled]="!structuredPersona()" 
                    class="px-6 py-3 rounded-full border font-bold transition-all flex items-center gap-2 disabled:opacity-50 hover:bg-black/5 border-[var(--vibe-accent)] text-[var(--vibe-accent)]">
              <app-icon name="psychology" [size]="20"></app-icon>
              {{ wf.t('crys.btn_refine') }}
            </button>
            
            <!-- Check (Next) -->
            <button (click)="goToCheck()" [disabled]="!structuredPersona()" 
                    class="px-6 py-3 rounded-full text-[var(--vibe-on-accent)] font-bold shadow-lg transition-all flex items-center gap-2 disabled:opacity-50 hover:opacity-90 bg-[var(--vibe-accent-bg)]">
              {{ wf.t('crys.btn_check') }}
...
export class CrystallizeComponent implements OnInit {
